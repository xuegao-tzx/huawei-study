...
public class UserInfoEN extends JFrame implements IData, ActionListener {
    ...
    UserInfoEN(String st) {
        ...
        setTitle("Admin " + st + "'s user information display area");
    }
    ...
    public void init(String st) {
        setLayout(null);
        Object[] title = {"Account", "Password", "Name", "Gender", "ID number", "Registration time"};//1.3.2.5.4.6
        setJmb(new JMenuBar());
        setJm(new JMenu("Function"));
        setMoadd(new JMenuItem("Mod"));
        getMoadd().addActionListener(this);
        setDelete(new JMenuItem("Del"));
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == getMoadd()) {
            getJFm().dispose();
            new ModedEN(getAdmin());
        } else if (e.getSource() == getDelete()) {
            getJFm().dispose();
            new DeletedEN(getAdmin());
        }
    }
